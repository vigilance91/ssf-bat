...
rem )
if not defined OFP (
    rem set "OFP=%PACKED_SIGNED_OD%/tmp.hex.ssf32"
    set /P OFP=Please enter the fully qualified or relative file path to write to 
)
rem 
rem exit /B %ERRORLEVEL%
...
if not defined _QUIET (
    if defined _D (
        if defined VERB (
            call :verboseLog "SSF encode string, packed signed hex" %LFP%
        ) else (
...
if defined CLI_FLAGS (
    ...
    %php5% %VSN_SSF_CLI_PHP56_DIR%/run.php --M=encode --F=stringtofile --source="%SRC%" --O="%OFP%" --privateKey=%PKP% --password=%PKPW% --hex %CLI_FLAGS%
) else (
    %php5% %VSN_SSF_CLI_PHP56_DIR%/run.php --M=encode --F=stringtofile --source="%SRC%" --O="%OFP%" --privateKey=%PKP% --password=%PKPW% --hex
)
