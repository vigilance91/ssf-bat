...
rem @brief VSN SSF32 batch file for automated genertion of project SSF32 base64 signtures
...
rem --h help mode
rem 
rem title "ssf32 - packed signed - encode string - base64.bat"
rem color 17
rem 
...
shift
goto :loop
rem @note if neither --base64 nor --base64 flag are specified, both encodings will be output
rem 
rem else if "%1"=="--S" (
...
rem 
if "%1"=="" (
    set /P OFP=Please enter the fully qualified or relative file path to write to 
    goto :continue
)
if not defined OFP (
   set "OFP=%~1"
...
set "ID=%cd%\_src"
rem 
rem set FP_LFP=%OD%\base64\files%XT%
rem set SRC_LFP=%OD%\base64\sources%XT%
rem 
set "LFP=%PACKED_SIGNED_OD%\ssfEncodeStringToFileHex%XT%"
...
rem packed
rem 
rem set "PACKED_UNSIGNED_HEX_LFP=%PACKED_SIGNED_OD%\base64%XT%"
rem set "PACKED_SIGNED_HEX_LFP=%PACKED_SIGNED_OD%\base64%XT%"
rem 
rem JSON OUTPUT PATHS
rem 
rem set "PACKED_UNSIGNED_HEX_JSON_FP=%PACKED_SIGNED_OD%\base64%JSON_XT%"
rem set "PACKED_SIGNED_HEX_JSON_FP=%PACKED_SIGNED_OD%\base64%JSON_XT%"
...
rem if COMPRESS (
rem     set "OFP=%PACKED_SIGNED_OD%/tmp.gz.base64.ssf32"
rem )
if not defined OFP (
    rem set "OFP=%PACKED_SIGNED_OD%/tmp.base64.ssf32"
    set /P OFP=Please enter the fully qualified or relative file path to write to 
)
rem 
rem exit /B %ERRORLEVEL%
...
rem %php5% %GLOBAL_PHARS_DIR%/ssf-cli-php56/run.php --M=encode --F=directory --source=%cd% --O=%LFP% --base64
...
rem 
rem %php5% %GLOBAL_PHARS_DIR%/ssf-cli-php56/run.php --M=encode --F=directory --source="%cd%\_src" --O=%PACKED_UNSIGNED_HEX_LFP% --base64
...
rem     echo SSF encode directory, packed signed base64 >> %LF%
...
rem echo base64 flag >> %LF%
if not defined _QUIET (
    if defined _D (
        if defined VERB (
            call :verboseLog "SSF encode string, packed signed base64" %LFP%
        ) else (
            echo SSF encode string, packed signed base64 >> %LF%
        )
...
if defined CLI_FLAGS (
    ...
    %php5% %VSN_SSF_CLI_PHP56_DIR%/run.php --M=encode --F=stringtofile --source="%SRC%" --O="%OFP%" --privateKey=%PKP% --password=%PKPW% --base64 %CLI_FLAGS%
) else (
    %php5% %VSN_SSF_CLI_PHP56_DIR%/run.php --M=encode --F=stringtofile --source="%SRC%" --O="%OFP%" --privateKey=%PKP% --password=%PKPW% --base64
)
...
rem )
rem %php5% %GLOBAL_PHARS_DIR%/ssf-cli-php56/run.php --M=encode --F=directory --source="%BUILD_DIR%" --O=%PACKED_SIGNED_HEX_LFP% --base64 --privateKey=%RSA_PRIVATE_KEY_FILE% --password=%UPW%
rem if %ERRORLEVEL% NEQ 0 (
rem     if defined _D (
