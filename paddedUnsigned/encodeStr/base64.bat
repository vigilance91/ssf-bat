...
rem 
rem if defined _CHUNK (
rem )
rem if defined _COMPRESS (
rem )
rem 
rem exit /B %ERRORLEVEL%
rem 
...
rem    call ::ssf32EncodeStringPaddedUnsignedBase64 "%SRC%" "%PACKED_UNSIGNED_HEX_LFP%" "%FLAGS%"
rem ) else(
call ::ssf32EncodeStringPaddedUnsignedBase64 "%SRC%" %LFP%
rem )
if %ERRORLEVEL% NEQ 0 (
    rem if defined _D (
...
exit /b
rem  
:ssf32EncodeStringPaddedUnsignedBase64
%php5% %VSN_SSF_CLI_PHP56_DIR%/run.php --M=encode --F=str --source="%~1" --base64 --pad
if %ERRORLEVEL% NEQ 0 (
