...
rem 
rem if not defined XT ( set "XT=.log" )
if not defined  XT_LOG ( set "XT_LOG=.log" )
...
set "ID=%cd%"
rem 
rem set FP_LFP=%OD%\hex\files%XT_LOG%
rem set SRC_LFP=%OD%\hex\sources%XT_LOG%
rem 
set "LFP=%PADDED_UNSIGNED_OD%\ssf32HexBat%XT_LOG%"
rem 
...
rem packed
rem 
set "PADDED_UNSIGNED_HEX_LFP=%PADDED_UNSIGNED_OD%\ssf32Hex%XT_LOG%"
rem 
rem JSON FILE PATHS
rem 
rem packed
rem 
set "PADDED_UNSIGNED_HEX_JSON_FP=%PADDED_UNSIGNED_OD%\ssf32Hex%XT_JSON%"
rem 
rem 
rem Log File constants
rem
set LF_PADDED_UNSIGNED_HEX=%PADDED_UNSIGNED_HEX_LFP% 2>&1
rem set LFP=%OD%\base64%XT_LOG%
set LF=%LFP% 2>&1
rem 
...
if defined _D (
    if not defined _QUIET (
        echo SSF encode directory, padded unsigned hex >> %LF_PADDED_UNSIGNED_HEX%
        rem echo to: "%PADDED_SIGNED_HEX_LFP%" >> %LF_PADDED_SIGNED_HEX%
        echo %VS_SEP% >> %LF_PADDED_UNSIGNED_HEX%
    )
)
...
for /R %cd% %%f in (
    ...
    echo %%f ssf32 = >> %LF_PADDED_UNSIGNED_HEX%
    %php5% %VSN_SSF_CLI_PHP56_DIR%/run%XT_PHP% --M=encode --F=fts --source=%%f --pad --hex >> %LF_PADDED_UNSIGNED_HEX%
    echo; >> %LF_PADDED_UNSIGNED_HEX%
    echo %VS_SEP% >> %LF_PADDED_UNSIGNED_HEX%
    rem 
    rem %php5% %VSN_SSF_CLI_PHP56_DIR%/run%XT_PHP% --M=encode --F=fts --source=%%f --pad --base64 --privateKey=%PKP% --password=%PKPW% >> %LF_PADDED_SIGNED_BASE64%
    rem call ssf32DirBase64.bat --pad --source "%cd%"
    rem call :ssf32DirPaddedUnsignedBase64 "%cd%" "%PADDED_UNSIGNED_BASE64_LFP%" "%PADDED_UNSIGNED_OD%\ssf32CDAllPHP_base64.log"
    if %ERRORLEVEL% NEQ 0 (
        if defined _D (
            if not defined _QUIET (
