...
if not defined _QUIET (
    if defined _D (
        if defined VERB (
            call :verboseLog "SSF encode string, padded signed hex" %LFP%
        ) else (
            echo SSF encode string, padded signed hex >> %LF%
        )
...
if defined CLI_FLAGS (
    ...
    %php5% %VSN_SSF_CLI_PHP56_DIR%/run.php --M=encode --F=str --source="%SRC%" --privateKey=%PKP% --password=%PKPW% --hex --pad %CLI_FLAGS%
) else (
    %php5% %VSN_SSF_CLI_PHP56_DIR%/run.php --M=encode --F=str --source="%SRC%" --privateKey=%PKP% --password=%PKPW% --hex --pad
)
