...
rem    call :ssf32EncodeStringPackedUnsignedHex "%SRC%" "%PACKED_UNSIGNED_HEX_LFP%" "%FLAGS%"
rem ) else(
call :ssf32EncodeStringPackedUnsignedHex "%SRC%" %LFP%
rem )
if %ERRORLEVEL% NEQ 0 (
    rem if defined _D (
...
(
    ...
    echo @brief VSN (C)(TM) ssf-bat::packedUnsigned::encdoeStr::hex.BAT
    echo %VS_SEP%
) >> %~1 2>&1
rem     
exit /b
...
exit /b
rem  
:ssf32EncodeStringPackedUnsignedHex
%php5% %VSN_SSF_CLI_PHP56_DIR%/run.php --M=encode --F=str --source="%~1" --hex
if %ERRORLEVEL% NEQ 0 (
