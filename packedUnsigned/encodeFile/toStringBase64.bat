...
) else if "%~1"=="--src" goto :parseSrc
rem ) else if "%~1"=="--pk" goto :parsePrivateKey
rem ) else if "%~1"=="--pw" goto :parsePassword
shift
goto loop
...
rem 
set "LFP=%PACKED_SIGNED_OD%\ssfEncodeFileToStringBase64%XT%"
...
rem     call :rmDirIfExists "%PACKED_UNSIGNED_OD%"
rem     goto dealloc
rem )
rem 
...
if not defined _QUIET (
    if defined _D (
        if defined VERB (
            call :verboseLog "SSF encode file to string, packed unsigned base64" %LFP%
        ) else (
            echo SSF encode string, packed unsigned hex >> %LF%
        )
...
if defined CLI_FLAGS (
    ...
    %php5% %VSN_SSF_CLI_PHP56_DIR%/run.php --M=encode --F=filetostring --source="%SRC%" --base64 %CLI_FLAGS%
) else (
    %php5% %VSN_SSF_CLI_PHP56_DIR%/run.php --M=encode --F=filetostring --source="%SRC%" --base64
)
