...
rem 
set "LFP=%OD%\cdAllssf32Base64Bat%XT%"
rem 
...
rem packed
rem 
set "PACKED_UNSIGNED_BASE64_LFP=%PACKED_UNSIGNED_OD%\ssf32Base64%XT%"
rem 
rem 
rem JSON FILE PATHS
rem 
rem packed
rem 
set "PACKED_UNSIGNED_BASE64_JSON_FP=%PACKED_UNSIGNED_OD%\ssf32Base64%XT_JSON%"
rem 
rem Log File constants
rem
set LF_PACKED_UNSIGNED_BASE64=%PACKED_UNSIGNED_BASE64_LFP% 2>&1
...
rem %php5% %VSN_SSF_CLI_PHP56_DIR%/run%XT_PHP% --M=encode --F=directory --source=%cd% --O=%LFP% --hex
...
rem 
rem %php5% %VSN_SSF_CLI_PHP56_DIR%/run%XT_PHP% --M=encode --F=directory --source="%cd%\_src" --O=%PACKED_UNSIGNED_HEX_LFP% --hex
rem if %ERRORLEVEL% NEQ 0 exit /B %ERRORLEVEL%
rem 
rem %php5% %VSN_SSF_CLI_PHP56_DIR%/run%XT_PHP% --M=encode --F=directory --source="%cd%\_src" --O=%PACKED_UNSIGNED_BASE64_LFP% --base64
rem if %ERRORLEVEL% NEQ 0 exit /B %ERRORLEVEL%
rem 
        
call :truncLog %PACKED_UNSIGNED_BASE64_LFP%

if defined _D (
    if not defined _QUIET (
        echo SSF encode directory, packed unsigned base64 >> %LF_PACKED_UNSIGNED_BASE64%
        echo %VS_SEP% >> %LF_PACKED_UNSIGNED_BASE64%
        echo encoding: %%f >> %LF_PACKED_UNSIGNED_BASE64%
        rem echo to: "%PACKED_SIGNED_HEX_LFP%" >> %LF_PACKED_SIGNED_HEX%
        echo %VS_SEP% >> %LF_PACKED_UNSIGNED_BASE64%
    )
)
rem 
rem echo base64 flag >> %LF_PACKED_SIGNED_BASE64%
rem 
for /R %cd% %%f in (
    %_CLI_EXT%
) do (
    rem %php5% -f %VSN_SSF_CLI_PHP56_DIR%/run%XT_PHP% -- --M=encode --F=f --source=%%f --hex --v --d=3
    echo %%f ssf32 = >> %LF_PACKED_UNSIGNED_BASE64%
    %php5% %VSN_SSF_CLI_PHP56_DIR%/run%XT_PHP% --M=encode --F=fts --source=%%f --base64 >> %LF_PACKED_UNSIGNED_BASE64%
    echo; >> %LF_PACKED_UNSIGNED_BASE64%
    echo %VS_SEP% >> %LF_PACKED_UNSIGNED_BASE64%
    
    if %ERRORLEVEL% NEQ 0 (
        if defined _D (
            if not defined _QUIET (
                echo Terminating script with error %ERRORLEVEL% >> %LF_PACKED_UNSIGNED_BASE64%
                echo %VS_SEP% >> %LF_PACKED_UNSIGNED_BASE64%
            )
        )
        goto dealloc
