...
rem --h help mode
rem 
rem title "ssf32 - packed signed - encode string - hex.bat"
rem color 17
rem 
rem 
rem ENV VARS
...
rem 
rem %GLOBAL_PHARS_DIR% - on Win32 systems this should be either "C:/phars" or "%USERPROFILE%/phars" or Unix systems this should be "/usr/local/bin/phars"
rem %GLOBAL_SCRIPTS_DIR% - on Win32 systems this should be either "C:/phars" or "%USERPROFILE%/_scripts" or Unix systems this should be "/usr/local/bin/_scripts"
rem 
...
rem 
if "%1"=="" (
    set /P SRC=Please enter a fully qualified or relative local file to encode, 
    goto :continue
)
...
shift
goto :loop
rem @note if neither --hex nor --hex flag are specified, both encodings will be output
rem 
rem else if "%1"=="--S" (
...
if not defined PACKED_SIGNED_OD (
    set "PACKED_SIGNED_OD=%OD%\packedUnsigned"
)
...
rem 
set "LFP=%PACKED_SIGNED_OD%\ssfEncodeFileToStringHex%XT%"
...
rem 
rem %php5% %GLOBAL_PHARS_DIR%/ssf-cli-php56/run.php --M=encode --F=directory --source="%cd%\_src" --O=%PACKED_UNSIGNED_BASE64_LFP% --hex
rem if %ERRORLEVEL% NEQ 0 exit /B %ERRORLEVEL%
rem 
rem 
rem if defined _D (
...
if not defined _QUIET (
    if defined _D (
        if defined VERB (
            call :verboseLog "SSF encode file to string, packed unsigned hex" %LFP%
        ) else (
            echo SSF encode string, packed signed hex >> %LF%
        )
...
rem packed unsigned
rem 
rem set "CMD_ARGS=--M=encode --F=str --source="%SRC%" --hex"
rem 
rem :main
if defined CLI_FLAGS (
    rem call :verboseLog "%CLI_FLAGS%" %LFP%
    %php5% %VSN_SSF_CLI_PHP56_DIR%/run.php --M=encode --F=filetostring --source="%SRC%" --hex %CLI_FLAGS%
) else (
    %php5% %VSN_SSF_CLI_PHP56_DIR%/run.php --M=encode --F=filetostring --source="%SRC%" --hex
)
...
echo @brief VSN (C)(TM) ssf32-packedUnsigned-hex.bat >> %~1 2>&1
echo %VS_SEP% >> %~1 2>&1
exit /b
rem 
rem 
