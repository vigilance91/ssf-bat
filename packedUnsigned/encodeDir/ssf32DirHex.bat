...
rem @brief VSN CHRONO CLI batch file for automated genertion of project SSF32 Hex signtures
...
if "%1"=="" (
    ...
    if "%1"=="--d" (
        ...
        if defined _E ( exit /B 1 ) else ( set "_E=--hex" )
        rem if not defined LFP ( set LFP=%OD%\ssf32\hex%XT% )
    ) else if "%~1"=="--base64" (
        if defined _E ( exit /B 1 ) else ( set "_E=--base64" )
        rem if not defined LFP ( set LFP=%OD%\ssf32\base64%XT% )
    ) else if "%~1"=="--pad" (
...
rem packed
rem 
set "PACKED_UNSIGNED_HEX_LFP=%PACKED_UNSIGNED_OD%\hex%XT%"
set "PACKED_SIGNED_HEX_LFP=%PACKED_SIGNED_OD%\hex%XT%"
rem 
rem padded
rem 
set "PADDED_UNSIGNED_HEX_LFP=%PADDED_UNSIGNED_OD%\hex%XT%"
set "PADDED_SIGNED_HEX_LFP=%PADDED_SIGNED_OD%\hex%XT%"
rem 
rem JSON OUTPUT PATHS
rem 
set "PACKED_UNSIGNED_HEX_JSON_FP=%PACKED_UNSIGNED_OD%\hex%JSON_XT%"
set "PACKED_SIGNED_HEX_JSON_FP=%PACKED_SIGNED_OD%\hex%JSON_XT%"
rem 
rem padded
rem 
set "PADDED_UNSIGNED_HEX_JSON_FP=%PADDED_UNSIGNED_OD%\hex%JSON_XT%"
set "PADDED_SIGNED_HEX_JSON_FP=%PADDED_SIGNED_OD%\hex%JSON_XT%"
...
if not defined _PAD (
    ...
    call :truncLog %PACKED_SIGNED_HEX_LFP%
    rem )
    rem         set UNSIGNED_LFP=%UNSIGNED_HEX_LFP%
    rem         set SIGNED_LFP=%SIGNED_HEX_LFP%
    rem 
    rem echo hex flag >> %LF%
    if defined _D (
        echo SSF encode directory, padded unsigned hex >> %PACKED_UNSIGNED_HEX_LFP% 2>&1
        call :logSrcDst "%BUILD_DIR%" "%PACKED_UNSIGNED_HEX_JSON_FP%" %PACKED_UNSIGNED_HEX_LFP%
        if %ERRORLEVEL% NEQ 0 (
            rem if defined _D (
    ...
    rem packed unsigned
    rem 
    call :ssf32DirPackedUnsignedHex "%BUILD_DIR%" "%PACKED_UNSIGNED_HEX_JSON_FP%" "%PACKED_UNSIGNED_HEX_LFP%"
    if %ERRORLEVEL% NEQ 0 (
        rem if defined _D (
        echo Terminating script with error %ERRORLEVEL% >> %LF%
        echo %VS_SEP% >> %LF%
    ...
    rem 
    if defined _D (
        rem echo SSF encode directory, packed signed hex >> %PACKED_SIGNED_HEX_LFP%
        call :logSrcDst "%BUILD_DIR%" "%PACKED_SIGNED_HEX_JSON_FP%" %PACKED_SIGNED_HEX_LFP%
        if %ERRORLEVEL% NEQ 0 (
            rem if defined _D (
            echo Terminating script with error %ERRORLEVEL% >> %LF%
            echo %VS_SEP% >> %LF%
        ...
        rem echo %VS_SEP% >> %LF%
        rem echo from: "%BUILD_DIR%" >> %LF%
        rem echo to: "%PACKED_SIGNED_HEX_JSON_FP%" >> %LF%
    )
    %php5% %VSN_SSF_CLI_PHP56_DIR%/run.php --M=encode --F=directory --source="%BUILD_DIR%" --O="%PACKED_SIGNED_HEX_JSON_FP%" --hex --privateKey=%RSA_PRIVATE_KEY_FILE% --password=%UPW%
    if %ERRORLEVEL% NEQ 0 (
        rem if defined _D (
        echo Terminating script with error %ERRORLEVEL% >> %LF%
        echo %VS_SEP% >> %LF%
    ...
    rem 
    rem if defined _D (
    call :truncLog %PADDED_UNSIGNED_HEX_LFP%
    call :truncLog %PADDED_SIGNED_HEX_LFP%
    rem )
    rem 
    if defined _D (
        echo SSF encode directory, padded unsigned hex >> %PADDED_UNSIGNED_HEX_LFP% 2>&1
        call :logSrcDst "%BUILD_DIR%" "%PADDED_SIGNED_HEX_JSON_FP%" %PADDED_UNSIGNED_HEX_LFP%
        if %ERRORLEVEL% NEQ 0 (
            rem if defined _D (
            echo Terminating script with error %ERRORLEVEL% >> %LF%
            echo %VS_SEP% >> %LF%
    ...
    rem 
    call :ssf32DirPaddedUnsignedHex "%BUILD_DIR%" "%PADDED_UNSIGNED_HEX_JSON_FP%" "%PADDED_UNSIGNED_HEX_LFP%"
    rem call :ssf32DirPaddedUnsignedHex "%BUILD_DIR%" "%PACKED_UNSIGNED_HEX_LFP%"
    if %ERRORLEVEL% NEQ 0 (
        rem if defined _D (
        echo Terminating script with error %ERRORLEVEL% >> %LF%
        echo %VS_SEP% >> %LF%
    ...
    rem 
    if defined _D (
        echo SSF encode directory, padded signed hex >> %PADDED_SIGNED_HEX_LFP%
        call :logSrcDst "%BUILD_DIR%" "%PADDED_SIGNED_HEX_JSON_FP%" %PADDED_SIGNED_HEX_LFP%
        if %ERRORLEVEL% NEQ 0 (
            rem if defined _D (
            echo Terminating script with error %ERRORLEVEL% >> %LF%
            echo %VS_SEP% >> %LF%
    ...
    %php5% %VSN_SSF_CLI_PHP56_DIR%/run.php --M=encode --F=directory --source="%BUILD_DIR%" --pad --hex --O=%PADDED_SIGNED_HEX_JSON_FP% --privateKey=%RSA_PRIVATE_KEY_FILE% --password=%UPW%
    if %ERRORLEVEL% NEQ 0 (
        rem if defined _D (
        echo Terminating script with error %ERRORLEVEL% >> %LF%
        echo %VS_SEP% >> %LF%
...
for /R %~dp0 %%f in (
    *.php
    ...
    echo %%f ssf32 = >> %LF%
    %php5% %GLOBAL_PHARS_DIR%/ssf-cli-php56/run.php --M=encode --F=fts --source=%%f --hex>> %LF%
    echo:>> %LF%
    rem echo %VS_SEP%>> %LF%
    rem )
    rem if %ERRORLEVEL% NEQ 0 exit /B %ERRORLEVEL%
...
rem
:logSrcDst
echo %VS_BANNER% >> %~3 2>&1
echo SSF encoding directory >> %~3 2>&1
echo source: "%~1" >> %~3 2>&1
echo destination: "%~2" >> %~3 2>&1
echo %VS_SEP% >> %~3 2>&1
exit /b
rem 
:truncLog
...
rem exit /b
rem 
:ssf32DirPackedUnsignedHex
%php5% %VSN_SSF_CLI_PHP56_DIR%/run.php --M=encode --F=directory --source="%~1" --O="%~2" --hex
if %ERRORLEVEL% NEQ 0 (
...
rem 
rem Padded convenience subroutines
rem 
:ssf32DirPaddedUnsignedHex
rem set "logPath=%cd%\_output\ssf32\paddedUnsigned\hex.log"
rem %php5% %SSF_CLI_PHP56_PHAR_DIR%/run.php --M=encode --F=directory --source="%~1" --O="%~2" --hex --pad
%php5% %VSN_SSF_CLI_PHP56_DIR%/run.php --M=encode --F=directory --source="%~1" --O="%~2" --hex --pad
if %ERRORLEVEL% NEQ 0 (
    echo Terminating script with error %ERRORLEVEL% >> %~3 2>&1
    echo %VS_SEP% >> %~3 2>&1
)
